...
export function ExcelImportPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(ax.response?.data?.error?.message || 'Import failed. Check the file format and try again.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6 animate-fade-in max-w-3xl">
      ...
      <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
        <div className="rounded-lg bg-blue-50 border border-blue-200 p-4 text-sm text-blue-700">
          The Excel file must contain a header row with at minimum a <strong>title</strong> column.
          Optional columns: source, description, country, amount, currency, deadline, funding_type, sector, url, eligibility_criteria.
        </div>
        ...
        <div {...getRootProps()} className={`rounded-xl border-2 border-dashed p-12 text-center cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-primary/5' : 'border-muted-foreground/30 hover:border-primary/50 bg-muted/20'}`}>
          ...
          <p className="text-sm text-muted-foreground">
            {file ? `${(file.size / 1024).toFixed(1)} KB` : 'Drag and drop an .xlsx file here, or click to browse'}
          </p>
        </div>
        ...
        {result && (
          <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-6 text-center">
            ...
            <p className="text-emerald-700">
              {result.created} created · {result.skipped} skipped (duplicates or missing title)
            </p>
          </div>
        )}
