import { useState } from 'react'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import api from '@/lib/axios'
import { CheckCircle, XCircle, AlertTriangle, Clock } from 'lucide-react'
import { formatDate } from '@/utils/formatDate'
import { formatCurrency } from '@/utils/formatCurrency'

export function ScrapingAlertsPage() {
  const queryClient = useQueryClient()
  const [page, setPage] = useState(1)

  // Assuming an endpoint that lists scraped opportunities pending review
  const { data, isLoading } = useQuery({
    queryKey: ['scraping-alerts', page],
    queryFn: () => api.get('/scraping/alerts/', { params: { page } }).then(r => r.data),
  })

  const publishMutation = useMutation({
    mutationFn: (id: number) => api.post(`/scraping/alerts/${id}/publish/`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['scraping-alerts'] }),
  })

  const rejectMutation = useMutation({
    mutationFn: (id: number) => api.post(`/scraping/alerts/${id}/reject/`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['scraping-alerts'] }),
  })

  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Scraping Alerts</h1>
        <p className="text-muted-foreground mt-1">Review recently scraped high-priority opportunities</p>
      </div>

      <div className="rounded-xl border bg-white shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
              <tr>
                <th className="px-6 py-3 font-medium">Priority</th>
                <th className="px-6 py-3 font-medium">Opportunity</th>
                <th className="px-6 py-3 font-medium">Amount</th>
                <th className="px-6 py-3 font-medium">Deadline</th>
                <th className="px-6 py-3 font-medium text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-primary border-t-transparent mx-auto" /></td></tr>
              ) : data?.results?.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-muted-foreground">No pending alerts.</td></tr>
              ) : (
                data?.results?.map((alert: any) => (
                  <tr key={alert.id} className="border-b last:border-0 hover:bg-muted/20">
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${
                        alert.priority === 'urgent' ? 'bg-red-100 text-red-700' :
                        alert.priority === 'high' ? 'bg-orange-100 text-orange-700' :
                        'bg-blue-100 text-blue-700'
                      }`}>
                         {alert.priority === 'urgent' ? <AlertTriangle className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                         {alert.priority}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-medium max-w-[250px] truncate" title={alert.title}>
                      <a href={alert.url} target="_blank" rel="noreferrer" className="text-primary hover:underline">{alert.title}</a>
                      <div className="text-xs text-muted-foreground mt-0.5">{alert.source} • {alert.country}</div>
                    </td>
                    <td className="px-6 py-4 font-mono">{alert.amount ? formatCurrency(alert.amount, alert.currency) : 'N/A'}</td>
                    <td className="px-6 py-4">{alert.deadline ? formatDate(alert.deadline) : 'N/A'}</td>
                    <td className="px-6 py-4 text-right">
                       <div className="flex items-center justify-end gap-2">
                         <button onClick={() => publishMutation.mutate(alert.id)} disabled={publishMutation.isPending} className="p-2 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100" title="Publish">
                           <CheckCircle className="h-4 w-4" />
                         </button>
                         <button onClick={() => rejectMutation.mutate(alert.id)} disabled={rejectMutation.isPending} className="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100" title="Dismiss">
                           <XCircle className="h-4 w-4" />
                         </button>
                       </div>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
      
      {data?.count > 0 && (
        <div className="flex justify-center gap-2 mt-4">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={!data?.previous} className="rounded-lg border px-4 py-2 text-sm disabled:opacity-50">Previous</button>
          <button onClick={() => setPage(p => p + 1)} disabled={!data?.next} className="rounded-lg border px-4 py-2 text-sm disabled:opacity-50">Next</button>
        </div>
      )}
    </div>
  )
}
